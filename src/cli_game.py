...
class CLIGame:
    ...
    def print_rules(self):
        ...
        for i, rule in enumerate(self.game_manager.rules, 1):
            print(
                f"{i}. {rule.name} (等级{rule.level}) - {(rule.description or '')[:30]}..."
            )
    ...
    async def game_loop(self):
        ...
        while self.running and self.game_manager.state:
            ...
            elif self.game_manager.state.phase in [
                GamePhase.MORNING_DIALOGUE,
                GamePhase.EVENING_DIALOGUE,
            ]:
    ...
    async def setup_phase(self):
        ...
        if choice == "1":
            ...
            self.save_game()
        elif (choice == "6" and not self.ai_enabled) or (
            choice == "7" and self.ai_enabled
        ):
            self.running = False
    ...
    async def manage_rules(self):
        ...
        elif (choice == "3" and not self.ai_enabled) or (
            choice == "4" and self.ai_enabled
        ):
            print("升级功能尚未实现")
            await asyncio.sleep(1)
        ...
        try:
            ...
            if rule:
                ...
                if self.game_manager.state.fear_points >= cost:
                    ...
                    print(
                        f"❌ 恐惧积分不足！需要 {cost}，当前只有 {self.game_manager.state.fear_points}"
                    )
    ...
    async def create_rule_from_template(self):
        ...
        try:
            ...
            if 0 <= idx < len(templates):
                ...
                # 创建规则
                rule = Rule(
                    id=f"rule_{len(self.game_manager.rules) + 1:03d}", **template
                )
    ...
    async def switch_mode(self):
        ...
        new_mode = (
            GameMode.IN_SCENE if current == GameMode.BACKSTAGE else GameMode.BACKSTAGE
        )
