...
class NPCMemory(BaseModel):
    ...
    def add_event(self, event_type: str, details: Dict[str, Any]):
        """添加事件记忆"""
        self.events.append(
            {"type": event_type, "details": details, "turn": details.get("turn", 0)}
        )
        # 只保留最近20条记忆
...
class NPC(BaseModel):
    ...
    def can_act(self) -> bool:
        """是否可以行动"""
        return (
            self.status not in [NPCStatus.DEAD, NPCStatus.INSANE] and self.stamina > 0
        )
    ...
    def decide_action(self, context: Dict[str, Any]) -> Optional[NPCAction]:
        ...
        if nearby_npcs:
            # 有信任的人在附近
            trusted_nearby = any(
                npc_id in self.relationships and self.relationships[npc_id] > 5
    ...
    def interact_with(self, other_npc: "NPC"):
        ...
        if other_npc.id not in self.relationships:
            ...
            initial_trust = (
                5
                + (self.personality.sociability - 5)
                + (other_npc.personality.sociability - 5) // 2
            )
    ...
    def choose_move_destination(
        self, current_area, available_areas, map_manager
    ) -> Optional[str]:
        """选择移动目的地"""
