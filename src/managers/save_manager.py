...
class SaveManager:
    ...
    def save_game(
        ...
        if not save_name:
            ...
            save_name = f"save_{state.game_id}_{timestamp}"
        ...
        save_data = {
            ...
            "description": description or f"Turn {state.turn}",
            "game_state": self._serialize_game_state(state),
            "managers": {
                "rules": self._serialize_rules(game_state_manager.rules),
                "npcs": self._serialize_npcs(state.npcs),
                "map": self._serialize_map(
                    game_state_manager.map_manager
                    if hasattr(game_state_manager, "map_manager")
                    else None
                ),
            },
            "statistics": game_state_manager.get_statistics()
            if hasattr(game_state_manager, "get_statistics")
            else {},
        }
    ...
    def list_saves(self) -> List[Dict[str, Any]]:
        ...
        for filepath in self.save_dir.glob(f"*{self.SAVE_EXTENSION}"):
            try:
                ...
                saves.append(
                    {
                        ...
                        "game_id": save_data.get("game_state", {}).get(
                            "game_id", "Unknown"
                        ),
                ...
                saves.append({"filename": filepath.name, "error": str(e)})
    ...
    def create_autosave(self, game_state_manager: GameStateManager) -> Optional[str]:
        ...
        try:
            ...
            # 自动存档使用特殊前缀
            save_name = f"autosave_{state.game_id}"
            description = f"自动存档 - 回合 {state.turn}"
    ...
    def _serialize_game_state(self, state: GameState) -> Dict[str, Any]:
        ...
        return {
            ...
            "events_history": getattr(
                state, "events_history", getattr(state, "event_log", [])
            )[
                -100:
            ],  # 只保存最近100条事件
            "statistics": {
        ...
        for rule in rules:
            serialized[rule.id] = (
                ...
                if hasattr(rule, "to_dict")
                    ...
                    "trigger": rule.trigger.__dict__
                    if hasattr(rule.trigger, "__dict__")
                    else {},
                    "effect": rule.effect.__dict__
                    if hasattr(rule.effect, "__dict__")
                    else {},
                    "requirements": rule.requirements,
                    "base_cost": getattr(rule, "base_cost", 0),
                    "level": rule.level,
                    "times_triggered": rule.times_triggered,
                    "loopholes": [lh.__dict__ for lh in rule.loopholes]
                    if hasattr(rule, "loopholes")
                    else [],
                    "active": rule.active,
                }
            )
    ...
    def _serialize_npcs(self, npcs: Dict[str, Any]) -> Dict[str, Dict]:
        """序列化NPC"""
        serialized = {}
            ...
            if isinstance(npc, dict):
                ...
                serialized[npc_id] = (
                    ...
                    if hasattr(npc, "to_dict")
                        ...
                        "personality": npc.personality.__dict__
                        if hasattr(npc.personality, "__dict__")
                        else {},
    ...
    def _serialize_map(self, map_manager: Optional[MapManager]) -> Optional[Dict]:
        ...
        return (
            map_manager.to_dict()
            if hasattr(map_manager, "to_dict")
            else {
                "areas": {
                    area_id: area.__dict__
                    for area_id, area in map_manager.areas.items()
                }
                if hasattr(map_manager, "areas")
                else {}
            }
        )
    ...
    def _deserialize_rules(self, data: Dict[str, Dict]) -> List[Rule]:
        """反序列化规则"""
        rules: List[Rule] = []
    ...
    def _deserialize_npcs(self, data: Dict[str, Dict]) -> Dict[str, Dict]:
        """反序列化NPC"""
