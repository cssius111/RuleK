...
class NPCBehavior:
    ...
    def _calculate_action_weights(
        self, npc: Dict, personality: Dict
        ...
        # 性格影响
        weights[NPCAction.INVESTIGATE] = weights.get(NPCAction.INVESTIGATE, 0.2) * (
            0.5 + personality["curiosity"]
        )
        weights[NPCAction.TALK] = weights.get(NPCAction.TALK, 0.15) * (
            0.5 + personality["sociability"]
        )
        weights[NPCAction.HIDE] = weights.get(NPCAction.HIDE, 0) * (
            1.5 - personality["courage"]
        )
        ...
        if location == "corridor":
            weights[NPCAction.TURN_AROUND] = (
                weights.get(NPCAction.TURN_AROUND, 0) + 0.05
            )
    ...
    def _determine_action_target(self, npc: Dict, action: NPCAction) -> Optional[str]:
        ...
        if action == NPCAction.MOVE:
            ...
            others = [
                o for o in others if o["id"] != npc["id"] and o.get("alive", True)
            ]
            if others:
        ...
        if decision.action == NPCAction.MOVE:
            if decision.target:
                ...
                result["messages"].append(
                    f"{npc['name']} 从 {old_location} 移动到 {decision.target}"
                )
            ...
            # 恢复体力
            self.game_manager.update_npc(
                npc["id"], {"stamina": min(100, npc.get("stamina", 100) + 20)}
            )
            ...
            # 增加怀疑度，可能发现线索
            self.game_manager.update_npc(
                npc["id"], {"suspicion": min(100, npc.get("suspicion", 0) + 10)}
            )
        ...
        stats: Dict[str, Any] = {
            ...
            "active_npcs": len(
                [
                    npc
                    for npc in (
                        self.game_manager.state.npcs.values()
        ...
        if self.game_manager.state:
            for npc in self.game_manager.state.npcs.values():
                if npc.get("alive", True):
                    ...
                    stats["location_distribution"][loc] = (
                        stats["location_distribution"].get(loc, 0) + 1
                    )
