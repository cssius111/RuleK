...
class ScreamHeardEffect(SideEffect):
    ...
    def apply(self, game_state: Any, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for location in affected_locations:
            ...
            fear_increase = int(15 * distance_modifier) if scream_type == "death" else int(10 * distance_modifier)
...
class SideEffectManager:
    ...
    def apply_effect(self, effect_name: str, game_state: Any, context: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """应用指定的副作用"""
        ...
        if result.get("success"):
            self.active_effects.append({
                ...
                "turn_applied": game_state.state.turn if hasattr(game_state, 'state') else 0,
                "context": context,
                "result": result
            })
    ...
    def apply_multiple_effects(self, effect_names: List[str], game_state: Any, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """应用多个副作用"""
        results = []
