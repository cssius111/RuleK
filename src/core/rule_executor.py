...
class RuleExecutor:
    ...
    def __init__(self, game_manager: GameStateManager):
        ...
        self.execution_history: DefaultDict[str, List[Dict[str, Any]]] = defaultdict(
            list
        )  # 规则执行历史
    ...
    def can_rule_trigger(self, rule: Rule, context: RuleContext) -> bool:
        ...
        if rule.trigger.time_range:
            if not self._check_time_range(
                context.current_time, rule.trigger.time_range
            ):
                return False
    ...
    def _check_extra_condition(self, condition: str, context: RuleContext) -> bool:
        ...
        condition_checks = {
            ...
            "alone": lambda: len(
                self.game_manager.get_npcs_in_location(
                    cast(str, context.actor_location)
                )
            )
            == 1,
    ...
    def _apply_rule_effects(self, rule: Rule, context: RuleContext, result: Dict):
        ...
        if result.get("fear_gained", 0) > 0:
            self.game_manager.add_fear_points(
                result["fear_gained"], f"规则触发: {rule.name}"
            )
        ...
        if result.get("sanity_loss"):
            new_sanity = max(
                0, context.actor.get("sanity", 100) - result["sanity_loss"]
            )
    ...
    def _apply_side_effect(self, side_effect: str, context: RuleContext):
        ...
        if side_effect == "blood_on_mirror":
            effect_context.update({"surface": "mirror", "message": "你看到了什么？"})
        ...
        result = self.side_effect_manager.apply_effect(
            side_effect, self.game_manager, effect_context
        )
        ...
        if result and result.get("success"):
            ...
            if result.get("fear_bonus"):
                self.game_manager.add_fear_points(
                    result["fear_bonus"], f"副作用: {side_effect}"
                )
        else:
    ...
    def detect_loopholes(self, npc: Dict, rule: Rule) -> Optional[str]:
        ...
        for loophole in rule.loopholes:
            ...
            # 根据破绽难度调整概率
            adjusted_chance = (
                detection_chance * (11 - loophole.discovery_difficulty) / 10
            )

            if random.random() < adjusted_chance:
                logger.info(
                    f"{npc['name']} 发现了规则 '{rule.name}' 的破绽: {loophole.description}"
                )
                return loophole.id
...
if __name__ == "__main__":
    ...
    test_rule = Rule(
        id="test_rule",
        name="测试规则",
        trigger=TriggerCondition(action="test_action", probability=0.8),
        effect=RuleEffect(type=EffectType.FEAR_GAIN, fear_gain=100),
    )
    ...
    context = RuleContext(
        actor=test_npc, action="test_action", game_state=game_manager.state.to_dict()
    )
