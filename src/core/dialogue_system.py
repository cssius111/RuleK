...
class DialogueSystem:
    ...
    async def generate_dialogue(self, npcs: List[Any], context: Dict[str, Any]) -> List[Dict[str, str]]:
        """生成NPC对话"""
        if len(npcs) < 2:
            return []
    ...
    async def generate_dialogue_round(self, npcs: List[Any], context: DialogueContext, 
                                    dialogue_type: DialogueType, turn: int) -> DialogueEntry:
        """生成一轮对话"""
        # 创建对话条目
        ...
        if templates and len(npcs) >= 2:
            ...
            for template in selected_templates:
                # 替换模板中的NPC名字
                dialogue_text = template.replace("{npc1}", npc1.name if hasattr(npc1, 'name') else str(npc1))
                dialogue_text = dialogue_text.replace("{npc2}", npc2.name if hasattr(npc2, 'name') else str(npc2))
